...
var KEY = "a910fc3f-20c9-4562-9410-58c997d20c42";
...
function updateDisplay() {
	...
	var headerFields = "<th>Item Name</th><th>Buy Order at</th><th>Sell Offer at</th><th>Quantity</th>";
	if (maxOffers > 1) {
	...
	calcData.forEach(function(item) { 
		...
		// the number of offers required to buy/sell that many items
		var rowFields = "<td>" + item.name + "</td><td>" + item.buyPrice.toFixed(1) + "</td><td>" + item.sellPrice.toFixed(1) + "</td><td>" + item.maxQuantity + "</td>";
		if (maxOffers > 1) {
...
function updateList() {
    setTimeout(getProductList, 60000)
};
